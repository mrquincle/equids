...
int CMessageClient::sendMessage(CMessage* msg)
{
//	acknowledge = 0;
	bool success = jockey_IPC.SendData(msg->type, (uint8_t*)msg->data, msg->len);
	if (!success) {
		fprintf(stderr,"Message not sent. Is there a connection?\n");
	} else {
...
{
	int p = atoi(port);
	bool success = jockey_IPC.Start(ip, p, false);
	if (!success) {
		fprintf(stderr,"Could not create a connection!\n");
	}
